...
*/
void veryCloseLogic(){
  if (millis() - lastClose_Time > MOTOR_VIB2STATIC_TIME && hasBeenClose ==1) {
    hasBeenClose = 0;
  }
...
void closeLogic(){
  ...
  if (millis() - lastVeryClose_Time > MOTOR_VIB2STATIC_TIME && hasBeenVeryClose == 1){
    hasBeenVeryClose = 0; 
  }

  // first alert the user he/she has entered the close mode
  if (millis() - enteringClose_Time > CHANGE_STATE_TIME && millis() - enteringClose_Time < CHANGE_STATE_TIME+ 300){
    hasBeenClose = 1;
  ...
  else if (hasBeenClose == 1){
    hasBeenClose = 1;
    hasBeenCloseLongTime = 0; 
    lastCloseLongTime_Time = millis();
    
    if(lastCloseLongTime_Time - lastClose_Time < MOTOR_VIB2VIBLOOP_TIME){
      lastClose_Time = millis();
      Serial.print("Entered Close State!!!!");
    }
