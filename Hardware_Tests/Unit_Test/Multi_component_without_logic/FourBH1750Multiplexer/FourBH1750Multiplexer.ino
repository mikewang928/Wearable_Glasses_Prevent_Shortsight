...
void loop() {
  ...
  for(int m = 0; m < 4; m++){ 
    ...
    readMux(m);
    Serial.println("after readMUX");
    init_BH1750(BH1750_1_ADDRESS, CONTINUOUS_HIGH_RES_MODE);
    Serial.print("after init");
    // delay(120);
    if(2==BH1750_Read(BH1750_1_ADDRESS))
      {
        SensorValue[m]=((buff[0]<<8)|buff[1])/1.2;
      }
    // delay(20);
  }
  Serial.print("after for loop!");
  Serial.print("Sensor_1 = "); Serial.print(SensorValue[0], DEC);
  Serial.print(" | Sensor_2 = "); Serial.print(SensorValue[1], DEC);
  Serial.print(" | Sensor_3 = "); Serial.print(SensorValue[2], DEC);
  Serial.print(" | Sensor_4 = "); Serial.println(SensorValue[3], DEC);
